...
const MovieFavoriteButton = ({position, movieId}) => {
  ...
  if (position === "tooltip") {
    return (
      <a className={`btn btn-outline ${favoriteIds?.indexOf(movieId) >= 0 ? 'active' : ''}`} onClick={() => handleFavClick()}>
...
export default memo(MovieFavoriteButton)